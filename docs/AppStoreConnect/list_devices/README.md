...
``app-store-connect list-devices [-h] [-s] [-v] [--no-color] [--log-stream {stderr, stdout}] [--platform {IOS, MAC_OS}] [--name DEVICE_NAME] [--status {DISABLED, ENABLED}] ``
...
|--status|DEVICE_STATUS|Status of the Device|DeviceStatus|DISABLED, ENABLED|
