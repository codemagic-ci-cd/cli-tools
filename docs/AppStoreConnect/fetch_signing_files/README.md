...
``app-store-connect fetch-signing-files [-h] [-s] [-v] [--no-color] [--log-stream {stderr, stdout}] [--platform {IOS, MAC_OS}] [--certificate-key PRIVATE_KEY] [--certificate-key-password PRIVATE_KEY_PASSWORD] [--p12-password P12_CONTAINER_PASSWORD] [--type {IOS_APP_ADHOC, IOS_APP_DEVELOPMENT, IOS_APP_INHOUSE, IOS_APP_STORE, MAC_APP_DEVELOPMENT, MAC_APP_DIRECT, MAC_APP_STORE, TVOS_APP_ADHOC, TVOS_APP_DEVELOPMENT, TVOS_APP_INHOUSE, TVOS_APP_STORE}] [--create] BUNDLE_ID_IDENTIFIER``
...
|--create||Whether to create the resource if it does not exist yet|bool|||
