...
### Usage
```bash
app-store-connect app-store-version-phased-releases enable [-h] [--log-stream STREAM] [--no-color] [--version] [-s] [-v]
    [--log-api-calls]
    ...
    [--state PHASED_RELEASE_STATE_OPTIONAL]
    APP_STORE_VERSION_ID
```
### Required arguments for action `enable`

##### `APP_STORE_VERSION_ID`


UUID value of the App Store Version
### Optional arguments for action `enable`
