...
### Usage
```bash
app-store-connect bundle-ids profiles [-h] [--log-stream STREAM] [--no-color] [--version] [-s] [-v]
    [--log-api-calls]
...
```
### Required arguments for action `profiles`

##### `--bundle-ids=BUNDLE_ID_RESOURCE_IDS`


Alphanumeric ID value of the Bundle ID. Multiple arguments
### Optional arguments for action `profiles`
