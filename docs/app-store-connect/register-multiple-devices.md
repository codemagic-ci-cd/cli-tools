...
**Register multiple devices for app development**
### Usage
```bash
app-store-connect register-multiple-devices [-h] [--log-stream STREAM] [--no-color] [--version] [-s] [-v]
    [--log-api-calls]
    ...
    --file-path FILE_PATH
```
### Required arguments for action `register-multiple-devices`

##### `--file-path=FILE_PATH`
The path to the file that contains the devices UDIDs. Each UDID should be on a new line.

### Optional arguments for action `register-multiple-devices`

##### `--platform=IOS | MAC_OS | UNIVERSAL | SERVICES`
Bundle ID platform. Default:&nbsp;`IOS`. This will apply for all devices

##### `--name=DEVICE_NAME`
Name of the Devices. Default: `Device from CLI`
