...
class GetLatestBuildNumberAction(GooglePlayBaseAction, metaclass=ABCMeta):
    ...
    def get_latest_build_number(
        ...
        self._log_get_package_action_started(requested_track_names)
        package_tracks = self.list_tracks(should_print=False)
        ...
        latest_build_number = self._get_latest_build_number(requested_track_names, track_version_codes)
    ...
    def _log_get_package_action_started(self, requested_tracks: Sequence[str]):
        if not requested_tracks:
            message = f'Get package "{self.package_name}" latest build number across all tracks'
        elif len(requested_tracks) == 1:
            message = f'Get package "{self.package_name}" latest build number from track "{requested_tracks[0]}"'
        else:
            formatted_specified_tracks = ", ".join(f'"{track}"' for track in requested_tracks)
            message = f'Get package "{self.package_name}" latest build number from tracks {formatted_specified_tracks}'
    ...
    def _get_latest_build_number(
        ...
        if not version_codes:
            error = self._get_missing_version_error(requested_tracks)
            raise GooglePlayError(error)
        return max(version_codes)

    def _get_missing_version_error(self, requested_tracks: Sequence[str]) -> str:
        if not requested_tracks:
            return f'Version code info is missing from all tracks for package "{self.package_name}"'
        elif len(requested_tracks) == 1:
            _track = requested_tracks[0]
            return f'Version code info is missing from track "{_track}" for package "{self.package_name}"'
        else:
            _tracks = ", ".join(f'"{track}"' for track in requested_tracks)
            return f'Version code info is missing from tracks {_tracks} for package "{self.package_name}"'
